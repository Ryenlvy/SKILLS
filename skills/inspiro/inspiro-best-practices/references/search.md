...
# Initialize Inspiro client
inspiro_client = AsyncInspiroClient("inspiro-YOUR_API_KEY")
...
async def fetch_and_gather():
    ...
    try:
        responses = await asyncio.gather(*(inspiro_client.search(q) for q in queries), return_exceptions=True)
...
Since Inspiro provides raw web data, you have full configurability to implement filtering and post-processing to meet your specific requirements.
...
For more details, please read the [full API reference](https://api.inspiro.top/documentation/api-reference/endpoint/search)